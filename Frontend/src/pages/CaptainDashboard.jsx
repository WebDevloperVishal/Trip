...
const CaptainDashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const [profileRes, usersTripsRes, upcomingRes] = await Promise.all([
          api.get("/captains/profile"),
          api.get("/captains/dashboard/users-trips"),
          api.get("/captains/dashboard/upcoming-trips"),
        ]);
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-orange-50 via-white to-teal-50 pt-24 pb-20 px-6">
            ...
            <tbody>
              {trips.slice(0, 8).map((trip) => (
                <tr key={trip._id} className="border-b hover:bg-orange-50">
                  <td className="p-3 flex items-center gap-2 font-semibold text-gray-800">
                    <img
                      src={
                        trip.user?.profileImage
                          ? `${BASE_URL}/${trip.user.profileImage.replace(/^\/+/, "")}`
                          : "https://cdn-icons-png.flaticon.com/512/3135/3135715.png"
                      }
                      alt="User Avatar"
                      className="w-8 h-8 rounded-full object-cover border border-gray-300"
                    />
                    {trip.user?.fullname?.firstname || "Unknown"}
                  </td>
      ...
      >
        <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2">
          <FaCheckCircle className="text-green-500" /> Upcoming Trips
        </h2>
          ...
          <table className="w-full text-left border-collapse text-gray-700">
            ...
            <tbody>
              {upcomingTrips.map((trip) => (
                <tr key={trip._id} className="border-b hover:bg-green-50">
                  <td className="p-3 flex items-center gap-2 font-semibold text-gray-800">
                    <img
                      src={
                        trip.user?.profileImage
